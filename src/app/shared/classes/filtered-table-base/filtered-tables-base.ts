...
import { PaginatedResponses } from '../../interfaces/models/paginated-response.model';
...
export abstract class FilteredTable<T> {
  ...
  abstract data: PaginatedResponses<T[]>;
  abstract _fetchData(page: number, hitsPerPAge?: number): void;

  handleApplyFilter(filters: Record<string, FilterActive>) {
    this._applyFilter(Object.values(filters));
  }
  ...
  _updateFilterText(): void {
    ...
    this.filtersActive.forEach(filter => {
      filterText = `${filterText}&${filter.name}=${filter.value}`;
    });

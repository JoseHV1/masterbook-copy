...
@Component({
  selector: 'app-policy-types-list',
  templateUrl: './policy-types-list.component.html',
  styleUrls: ['./policy-types-list.component.scss'],
})
export class PolicyTypesListComponent
  extends FilteredTable<PolicyTypeModel>
  implements OnInit, OnDestroy
{
  filterConfig!: FilterWrapperModel;

  data: PaginatedResponse<PolicyTypeModel[]> = {
    records: [],
    total_records: 0,
    page: 0,
    limit: 10,
  ...
  constructor(
    ...
    super();
    this.filterConfig = this._policyTypes.getAdminFilters();
  ...
  goToNew(): void {
    this._router.navigateByUrl('portal-admin/policy-settings/policy-types/new');
  }

  confirmDelete(policyType: PolicyTypeModel): void {
    if (!policyType.tenants?.length) {
      this._ui
        ...
        .subscribe((confirmed: boolean) => {
          if (confirmed) this._executeDelete(policyType._id);
        });
      return;
    }
    ...
    this._dialog
      ...
      .subscribe((result: TenantScopeModalResult | null) => {
        if (result) this._executeTenantDelete(policyType._id, result.tenantIds);
      });
  }
  ...
  openTenantStatusModal(policyType: PolicyTypeModel): void {
    if (!policyType.tenants?.length) return;
    ...
    this._dialog
      ...
      .pipe(take(1))
      .subscribe((result: TenantScopeModalResult | null) => {
        if (result?.status) this._executeTenantStatusUpdate(policyType._id, result.tenantIds, result.status);
      });
  }

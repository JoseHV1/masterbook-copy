...
@Component({
  selector: 'app-policy-categories-list',
  templateUrl: './policy-categories-list.component.html',
  styleUrls: ['./policy-categories-list.component.scss'],
})
export class PolicyCategoriesListComponent
  extends FilteredTable<PolicyCategoryModel>
  implements OnInit, OnDestroy
{
  filterConfig!: FilterWrapperModel;

  data: PaginatedResponse<PolicyCategoryModel[]> = {
    records: [],
    total_records: 0,
    page: 0,
    limit: 10,
  ...
  constructor(
    ...
    super();
    this.filterConfig = this._categories.getAdminFilters();
  ...
  goToNew(): void {
    this._router.navigateByUrl('portal-admin/policy-settings/policy-categories/new');
  }

  confirmDelete(category: PolicyCategoryModel): void {
    if (!category.tenants?.length) {
      this._ui
        ...
        .subscribe((confirmed: boolean) => {
          if (confirmed) this._executeDelete(category._id);
        });
      return;
    }
    ...
    this._dialog
      ...
      .subscribe((result: TenantScopeModalResult | null) => {
        if (result) this._executeTenantDelete(category._id, result.tenantIds);
      });
  }
  ...
  openTenantStatusModal(category: PolicyCategoryModel): void {
    if (!category.tenants?.length) return;
    ...
    this._dialog
      ...
      .pipe(take(1))
      .subscribe((result: TenantScopeModalResult | null) => {
        if (result?.status) this._executeTenantStatusUpdate(category._id, result.tenantIds, result.status);
      });
  }
